<footer class="site-footer">
    ...
    <div id="storeModal" class="modal" style="display: none;">
        <div class="modal-content">
            ...
            <form id="storeRegistrationForm" class="store-form" action="{{ route('store.register') }}" method="POST"
                enctype="multipart/form-data">
                @csrf
           <div class="form-group">
    <label for="storeName">نام فروشگاه:</label>
    <input type="text" id="storeName" name="name" value="{{ old('name') }}">
    @error('name')
        <div class="alert alert-danger" style="color: red; font-size: 0.9rem;">{{ $message }}</div>
    @enderror
</div>

                <div class="form-group">
                    <label for="storePhone">شماره تماس:</label>
                    <input type="tel" id="storePhone" name="phone_number" value="{{ old('phone_number') }}">
                       @error('phone_number')
        <div class="alert alert-danger" style="color: red; font-size: 0.9rem;">{{ $message }}</div>
    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="storeAddress" name="address">{{ old('address') }}</textarea>
                                           @error('address')
        <div class="alert alert-danger" style="color: red; font-size: 0.9rem;">{{ $message }}</div>
    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="productTypes" name="productTypes" placeholder="لطفاً انواع محصولات خود را شرح دهید">{{ old('productTypes') }}</textarea>
                                           @error('productTypes')
        <div class="alert alert-danger" style="color: red; font-size: 0.9rem;">{{ $message }}</div>
    @enderror
                </div>
...
<script>
    document.getElementById('openStoreModal').addEventListener('click', function () {
        document.getElementById('storeModal').style.display = 'block';
    });
